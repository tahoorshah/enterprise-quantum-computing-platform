...
| **OWASP Top 10** | A01 Broken Access Control, A07 Auth Failures, A06 Vulnerable Components | JWT auth on every route verified against a real user store (A01, A07); Pydantic schema validation on all inputs; pip-audit in CI (A06) | Partial - no rate limiting; input validation via schemas only |
